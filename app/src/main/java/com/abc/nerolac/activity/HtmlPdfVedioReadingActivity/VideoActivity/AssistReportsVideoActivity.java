...
public class AssistReportsVideoActivity extends AppCompatActivity {

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_assist_reports_video);

        VideoView videoView;

        Intent intentAssistReports = getIntent();
        AssistModel assistModel = intentAssistReports.getParcelableExtra("AssistModel");

        String videoName = assistModel.getName();

        String appDirectory =  Environment.getExternalStorageDirectory().getAbsolutePath();

        File file = new File(appDirectory+"/KnowledgeBase/SELF TRAINING/Reports/"+videoName+".mp4");

        String file1 = appDirectory+"/KnowledgeBase/SELF TRAINING/Reports/"+videoName+".mp4";
