...
public class AssistTransactionVisitVideoActivity extends AppCompatActivity {

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_assist_transaction_visit);

        VideoView videoView;

        Intent intentAssistTransaction = getIntent();
        AssistModel assistModel = intentAssistTransaction.getParcelableExtra("AssistModel");

        String videoName = assistModel.getName();

        String appDirectory =  Environment.getExternalStorageDirectory().getAbsolutePath();

        File file = new File(appDirectory+"/KnowledgeBase/SELF TRAINING/Transaction/Visits/"+videoName+".mp4");


        String file1 = appDirectory+"/KnowledgeBase/SELF TRAINING/Transaction/Visits/"+videoName+".mp4";
